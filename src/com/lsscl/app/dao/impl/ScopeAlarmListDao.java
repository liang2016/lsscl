...
public class ScopeAlarmListDao extends AppDao {
	...
	public static final String select_scopeAlarmList = "with ScopeTree AS "
			...
			+ "where e.typeId!=3 and e.typeId!=4 and( e.ackTs is null or (e.rtnApplicable='Y' and e.rtnTs is null)) "
			+ "and s.scopetype = 3 and e.emailHandler = ? and e.activeTs > ?)tt "
			+ "where rn>=? and rn<? ";
	public static final String select_scopeAlarmList2 = "with ScopeTree AS "
			+ "( "
			+ "SELECT id,scopename,scopetype,parentid FROM scope "
			+ "WHERE id = ? "
			+ "UNION ALL "
	...
	public RSP getRSP(QC qc) {
		...
		if(!"0".equals(level)){
			events = ejt.query(select_scopeAlarmList, new Object[] { scopeId,
					level, time, start, end }, new ScopeEventMapper());
		}else{
			events = ejt.query(select_scopeAlarmList2, new Object[] { scopeId,
