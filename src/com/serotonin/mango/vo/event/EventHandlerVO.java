...
public class EventHandlerVO implements Serializable, ChangeComparable<EventHandlerVO>, JsonSerializable {
    ...
    public boolean isSendContory() {
		return sendContory;
	}

	public void setSendContory(boolean sendContory) {
		this.sendContory = sendContory;
	}

	private boolean sendContory;
    ...
    private boolean sendEscalation2;
    private int escalationDelayType2;
    private int escalationDelay2;
    private List<RecipientListEntryBean> escalationRecipients2;
    ...
    public boolean isSendEscalation2() {
		return sendEscalation2;
	}

	public void setSendEscalation2(boolean sendEscalation2) {
		this.sendEscalation2 = sendEscalation2;
	}

	public int getEscalationDelayType2() {
		return escalationDelayType2;
	}

	public void setEscalationDelayType2(int escalationDelayType2) {
		this.escalationDelayType2 = escalationDelayType2;
	}

	public int getEscalationDelay2() {
		return escalationDelay2;
	}

	public void setEscalationDelay2(int escalationDelay2) {
		this.escalationDelay2 = escalationDelay2;
	}

	public List<RecipientListEntryBean> getEscalationRecipients2() {
		return escalationRecipients2;
	}

	public void setEscalationRecipients2(
			List<RecipientListEntryBean> escalationRecipients2) {
		this.escalationRecipients2 = escalationRecipients2;
	}
...
//            if(useSMS){
//            	response.addGenericMessage("eventHandlers.useSMS");
//            }
            if (sendEscalation) {
            ...
            if (sendEscalation2) {
                if (escalationDelay2 <= 0)
                    response.addContextualMessage("escalationDelay2", "eventHandlers.escalDelayError");
                if (escalationRecipients2.isEmpty())
                    response.addGenericMessage("eventHandlers.noEscalRecips");
            }
    ...
    public void addProperties(List<LocalizableMessage> list) {
        ...
        AuditEventType.addPropertyMessage(list, "common.disableChange", disableChange);
        ...
        if (handlerType == TYPE_SET_POINT) {
            ...
            AuditEventType.addPropertyMessage(list, "eventHandlers.useSMS",useSMS);
            AuditEventType.addPropertyMessage(list, "eventHandlers.sendContory", sendContory);
            ...
            AuditEventType.addPropertyMessage(list, "eventHandlers.escal", sendEscalation2);
            if (sendEscalation2) {
                AuditEventType
                        .addPeriodMessage(list, "eventHandlers.escalPeriod", escalationDelayType2, escalationDelay2);
                AuditEventType.addPropertyMessage(list, "eventHandlers.escalRecipients",
                        createRecipientMessage(escalationRecipients2));
            }
    ...
    public void addPropertyChanges(List<LocalizableMessage> list, EventHandlerVO from) {
        ...
        AuditEventType.maybeAddPropertyChangeMessage(list, "common.disableChange",from.disableChange,disableChange);
        ...
        if (handlerType == TYPE_SET_POINT) {
            ...
            AuditEventType.maybeAddPropertyChangeMessage(list, "eventHandlers.useSMS", from.useSMS,
                    useSMS);
            AuditEventType.maybeAddPropertyChangeMessage(list, "eventHandlers.sendContory", from.sendContory,
                    sendContory);
            ...
            AuditEventType.maybeAddPropertyChangeMessage(list, "eventHandlers.escal", from.sendEscalation2,
                    sendEscalation2);
            AuditEventType.maybeAddPeriodChangeMessage(list, "eventHandlers.escalPeriod", from.escalationDelayType2,
                    from.escalationDelay2, escalationDelayType2, escalationDelay2);
            AuditEventType.maybeAddPropertyChangeMessage(list, "eventHandlers.escalRecipients",
                    createRecipientMessage(from.escalationRecipients2), createRecipientMessage(escalationRecipients2));
    ...
    private void writeObject(ObjectOutputStream out) throws IOException {
        ...
        if (handlerType == TYPE_SET_POINT) {
            ...
            out.writeBoolean(useSMS);
            out.writeBoolean(sendContory);
            ...
            out.writeBoolean(sendEscalation2);
            out.writeInt(escalationDelayType2);
            out.writeInt(escalationDelay2);
            out.writeObject(escalationRecipients2);
    ...
    private void readObject(ObjectInputStream in) throws IOException, ClassNotFoundException {
        ...
        if (ver == 1) {
            ...
            if (handlerType == TYPE_SET_POINT) {
                ...
                useSMS = in.readBoolean();
                sendContory = in.readBoolean();
                ...
                sendEscalation2 = in.readBoolean();
                escalationDelayType2 = in.readInt();
                escalationDelay2 = in.readInt();
                escalationRecipients2 = (List<RecipientListEntryBean>) in.readObject();
    ...
    public void jsonSerialize(Map<String, Object> map) {
        ...
        map.put("xid", xid);
        map.put("disableChange", disableChange);
        ...
        if (handlerType == TYPE_SET_POINT) {
            ...
            map.put("useSMS", useSMS);
            map.put("sendContory", sendContory);
            ...
            map.put("sendEscalation2", sendEscalation2);
            if (sendEscalation) {
                map.put("escalationDelayType2", Common.TIME_PERIOD_CODES.getCode(escalationDelayType2));
                map.put("escalationDelay2", escalationDelay2);
                map.put("escalationRecipients2", escalationRecipients2);
            }
    ...
    public void jsonDeserialize(JsonReader reader, JsonObject json) throws JsonException {
        ...
        if (handlerType == TYPE_SET_POINT) {
            ...
            Boolean b = json.getBoolean("useSMS");
            if(b!=null)
            	useSMS=b;
            b=json.getBoolean("sendContory");
            if(b!=null)
            	sendContory=b;
            
            b = json.getBoolean("sendEscalation");
            if (b != null)
                sendEscalation = b;
            ...
            b = json.getBoolean("sendEscalation2");
            if (b != null)
                sendEscalation2 = b;

            if (sendEscalation2) {
                text = json.getString("escalationDelayType2");
                if (text != null) {
                    escalationDelayType2 = Common.TIME_PERIOD_CODES.getId(text);
                    if (escalationDelayType2 == -1)
                        throw new LocalizableJsonException("emport.error.invalid", "escalationDelayType2", text,
                                Common.TIME_PERIOD_CODES.getCodeList());
                }

                Integer i = json.getInt("escalationDelay2");
                if (i != null)
                    escalationDelay2 = i;

                JsonArray jsonEscalationRecipients2 = json.getJsonArray("escalationRecipients2");
                if (jsonEscalationRecipients2 != null)
                    escalationRecipients2 = reader.readPropertyValue(jsonEscalationRecipients2, List.class,
                            RecipientListEntryBean.class);
            }
