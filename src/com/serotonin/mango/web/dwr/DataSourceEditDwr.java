...
public class DataSourceEditDwr extends DataSourceListDwr {
    ...
    //
    @MethodFilter
    public DwrResponseI18n editInit(int dsid) {
        DwrResponseI18n response = new DwrResponseI18n();
        response.addData("points", getDataPointWhitoutAcp(dsid));
        response.addData("acpList", getAcps(dsid));
        response.addData("pointsOfAcpList", getPointsOfAcpList(dsid));	
        response.addData("alarms", getAlarms());
        response.addData("systemList", getSystems(dsid));
        response.addData("pointsOfSystemList", getPointsOfSystemList(dsid));
        return response;
    }
    ...
    private DwrResponseI18n tryDataSourceSave(DataSourceVO<?> ds) {
        ...
        if (!response.getHasMessages()) {
            ...
            //这里要修改数据源下的点
            for (DataPointVO dataPoint : dataPointDao.getDataPoints(ds.getId(), null)) {

            	dataPoint.setDeviceName(ds.getName());
    ...
    public List<DataPointVO> getPoints() {
        ...
        List<DataPointVO> points = getDataPointWhitoutAcp(ds.getId());
        if (points != null)
            Collections.sort(points, DataPointNameComparator.instance);
        return points;
    }
    //获取数据源下的所有空压机的集合
    @MethodFilter
    public List<ACPVO> getAcps(int dsid) {
        User user = Common.getUser();
        if (user == null)
            return null;
        ...
        List<ACPVO> acps = new ACPDao().findAcpsByDataSourceId(dsid);
        if(acps!=null){
    ...
    //获取数据源下跌所有系统点的集合的集合
    public List<ACPVO> getSystems(int dsid) {
        User user = Common.getUser();
        if (user == null)
            return null;
        ...
        List<ACPVO> acps = new ACPDao().findSystemsByDataSourceId(dsid);
        if(acps!=null){
        	Collections.sort(acps, AcpNameComparator.instance);
        }
        return acps;
    ...
    // 保存或者修改一个空压机并获取最新的空压机集合
    public DwrResponseI18n editACP(int dsid,int id,String acpname,String xid,int typeId,int offset,int factoryId,int type,int volume,int pressure){
    	User user = Common.getUser();
    	ACPDao acpDao = new ACPDao();
    	ACPVO acpvo = new ACPVO();
    	int acpid = -1;
    	int count = 0;
    	acpvo.setAcpname(acpname);
		acpvo.setXid(xid);
		ACPTypeVO acpTypeVO = new ACPTypeVO();
		acpTypeVO.setId(typeId);
		acpvo.setAcpTypeVO(acpTypeVO);
		acpvo.setOffset(offset);
		acpvo.setType(type);
		acpvo.setVolume(volume);
		acpvo.setPressure(pressure);
		if(factoryId==-1){//后期从权限中读取
			acpvo.setFactoryId(user.getCurrentScope().getId());
		}else{
			acpvo.setFactoryId(factoryId);
		}
		acpvo.setId(id);
		if(id==Common.NEW_ID){
			acpid = acpDao.save(acpvo, dsid);
		}else{
			acpDao.update(acpvo,dsid);
		}
		DwrResponseI18n response = new DwrResponseI18n();
		if(type==ACPVO.ACP_TYPE){
			response.addData("acpid",acpid);
			response.addData("acpList", getAcps(dsid));
			response.addData("pointsOfAcpList", getPointsOfAcpList(dsid));	
		}else{
			response.addData("systemid",acpid);
			response.addData("systemList", getSystems(dsid));
			response.addData("pointsOfSystemList", getPointsOfSystemList(dsid));
		}
        return response;
    ...
    /**
     * 是否已经配置了属性，没有配置则不能被添加
     * @param typeId 型号Id
     * @return 此型号的空压机是否可以被添加
     */
    public boolean typeValidate(int typeId){
    	return new ACPTypeDao().hasAttr(typeId);
    }
    ...
    // 获取当前ID的空压机信息
    @MethodFilter
    public DwrResponseI18n getACP(int acpId) {
    	DwrResponseI18n response = new DwrResponseI18n();
    	if(acpId!=-1){
    		ACPVO acp =  new ACPDao().findById(acpId);
    		response.addData("acp", acp);
    	}
    	response.addData("acptypeList",new ACPTypeDao().findByType(ACPTypeVO.ACP_TYPE));
        return response;
    }
    
    // 获取当前ID的系统信息
    @MethodFilter
    public DwrResponseI18n getSystem(int systemId) {
    	DwrResponseI18n response = new DwrResponseI18n();
    	if(systemId!=-1){
    		ACPVO acp =  new ACPDao().findById(systemId);
    		response.addData("system", acp);
    	}
    	response.addData("systemtypeList",new ACPTypeDao().findByType(ACPTypeVO.SYSTEM_TYPE));
        return response;
    }
    
    //查询所有系统型号
    public List<ACPTypeVO> getSystemTypes(){
    	return new ACPTypeDao().findByType(ACPTypeVO.SYSTEM_TYPE);
    }
     ...
     */
    @MethodFilter
    public DwrResponseI18n deleteACP(int dsid,int id) {
    	ACPDao acpDao = new ACPDao();
    	acpDao.delete(id);
		DwrResponseI18n response = new DwrResponseI18n();
        response.addData("acpList", getAcps(dsid));
    ...
    public DwrResponseI18n deleteSystem(int dsid,int id) {
    	ACPDao acpDao = new ACPDao();
    	acpDao.delete(id);
		DwrResponseI18n response = new DwrResponseI18n();
        response.addData("systemList", getSystems(dsid));
    ...
    public DwrResponseI18n saveVirtualDataSource(String name, String xid, int updatePeriods, int updatePeriodType) {
    	User user = Common.getUser();
    	VirtualDataSourceVO ds = (VirtualDataSourceVO) user.getEditDataSource();
        ds.setXid(xid);
        ds.setName(name);
        ds.setUpdatePeriods(updatePeriods);
        ds.setUpdatePeriodType(updatePeriodType);
        ds.setFactoryId(user.getCurrentScope().getId());
    ...
    public DwrResponseI18n saveModbusIpDataSource(String name, String xid, int updatePeriods, int updatePeriodType,
    	...
    	User user = Common.getUser();
        ModbusIpDataSourceVO ds = (ModbusIpDataSourceVO) user.getEditDataSource();
    ...
    public DwrResponseI18n saveMetaDataSource(String name, String xid) {
        ...
        ds.setXid(xid);
        ds.setName(name);
        ds.setFactoryId(Common.getUser().getCurrentScope().getId());
        return tryDataSourceSave(ds);
    }
    ...
    // 验证ACP是否合法，xid字段
    public boolean validateAcp(int acpid,String xid){
    	 return new ACPDao().validateXid(acpid,xid); 
    }
    
    /**
     * 获取空压机集合和空压机集合下的数据点集合 
     * @param dsid 数据源ID 
     * @return 空压机集合和空压机集合下的数据点集合 
     */
    public DwrResponseI18n getAcpsAndPoints(int dsid) {
        DwrResponseI18n response = new DwrResponseI18n();
        response.addData("acpList", getAcps(dsid));	
        response.addData("pointsOfAcpList", getPointsOfAcpList(dsid));	
        return response;
    ...
    /**
     * 获取该数据源下所有空压机下的点的集合的集合
     * @param dsid 数据源ID
     * @return 点的集合的集合
     */
    private List<List<DataPointVO>> getPointsOfAcpList(int dsid){
    	DataPointDao dpDao =  new DataPointDao();
        //空压机集合
        List<ACPVO> acpList = getAcps(dsid);
        //所有空压机的Id的数组
        if(acpList==null){
        	return null;
        }else{
        	...
        	//所有（空压机下的点的集合）的集合
        	List<List<DataPointVO>> pointsOfAcpList = new ArrayList<List<DataPointVO>>();
        	for(int i =0;i<acpList.size();i++){
        		acpIds[i]=acpList.get(i).getId();
        		List<DataPointVO> pointOfAcpList = dpDao.getDataPointByAcpId(acpIds[i],dsid);
        		pointsOfAcpList.add(pointOfAcpList);
        	}
        	return  pointsOfAcpList;
        }
    }
     ...
     * @return 点的集合的集合
     */
    private List<List<DataPointVO>> getPointsOfSystemList(int dsid){
    	DataPointDao dpDao =  new DataPointDao();
        //空压机集合
        List<ACPVO> systemList = getSystems(dsid);
        //所有空压机的Id的数组
        if(systemList==null){
        	return null;
        }else{
        	...
        	//所有（空压机下的点的集合）的集合
        	List<List<DataPointVO>> pointsOfSystemList = new ArrayList<List<DataPointVO>>();
        	for(int i =0;i<systemList.size();i++){
        		systemIds[i]=systemList.get(i).getId();
        		List<DataPointVO> pointOfSystemList = dpDao.getDataPointByAcpId(systemIds[i],dsid);
        		pointsOfSystemList.add(pointOfSystemList);
        	}
        	return  pointsOfSystemList;
        }
    }
     ...
     * @return xid
     */
    public String getACPUniqueXid(){
    	return new ACPDao().getUniqueXid();    	
    }
     ...
     * @param acpId 空压机点的ID
     */
    public DwrResponseI18n toggleAcp(int acpId,int dataSourceId,boolean enabled){
    	DwrResponseI18n response = new DwrResponseI18n();
    	DataPointDao dataPointDao = new DataPointDao();
    	//获取该空压机下的所有点
    	List<DataPointVO> dataPoints = dataPointDao.getDataPointByAcpId(acpId,dataSourceId);
    	//更新这些点的状态
    	...
    	response.addData("acpList", getAcps(dataSourceId));
        response.addData("pointsOfAcpList", getPointsOfAcpList(dataSourceId));	
        return response;
    }
     ...
     * 禁用或启用一个系统下的所有点
     */
    public DwrResponseI18n toggleSystem(int systemId,int dataSourceId,boolean enabled){
    	DwrResponseI18n response = new DwrResponseI18n();
    	DataPointDao dataPointDao = new DataPointDao();
    	//获取该系统下的所有点
    	List<DataPointVO> dataPoints = dataPointDao.getDataPointByAcpId(systemId,dataSourceId);
    	//更新这些点的状态
    	RuntimeManager runtimeManager = Common.ctx.getRuntimeManager();
    	for(DataPointVO dataPoint:dataPoints){
            Permissions.ensureDataSourcePermission(Common.getUser(), dataPoint.getDataSourceId());
    	...
    	response.addData("systemList", getSystems(dataSourceId));
        response.addData("pointsOfSystemList", getPointsOfSystemList(dataSourceId));
        return response;
    }
    /**
     * 获得所有空压机型号
     * @return 空压机型号列表或者错误信息
     */
    public DwrResponseI18n getAcpsByFactory(int factoryId){
       	DwrResponseI18n response = new DwrResponseI18n();
    	response.addData("acptypeList",new ACPDao().getAllAcpByFactoryId(factoryId));
    	return response;
    }
    /**
     * 根据空压机型号id查询meta数据点的模版配置
     * @param acpTypeId
     * @return meta数据点模版
     */
	...
	/**
	 * 根据模版id查询出模版数据，并且根据模版进行对应型号数据点
	 * @param modelId 模版id
	 * @return DwrResponseI18n
	 */
	public DwrResponseI18n getPointModel(int modelId,int acpId) {
		DwrResponseI18n response = new DwrResponseI18n();
		AcpMetaDao acpMetaDao=new AcpMetaDao();
		ACPDao acpDao=new ACPDao();
		AcpMetaVo metaModel=acpMetaDao.findMetaPoint(modelId);
		MetaPointLocatorVO pointLocator=(MetaPointLocatorVO)metaModel.getDp().getPointLocator();
		...
		List<IntValuePair> newContext=new ArrayList<IntValuePair>();
		for(IntValuePair point : context){
			int key=acpDao.getDataPointId(point.getKey(),acpId);
			if(key!=-1){
				newContext.add(new IntValuePair(key,point.getValue()));
			}
			else{
				response.addMessage(new LocalizableMessage(
						"meta.modle.error"));
		...
		pointLocator.setContext(newContext);
		response.addData("metaPoint",metaModel);
		return response;
	}
}
